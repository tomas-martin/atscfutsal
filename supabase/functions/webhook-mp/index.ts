...
// Al principio del archivo, agregá esta función:
...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:Arial,sans-serif;">
  <div style="max-width:560px;margin:40px auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.1);">
    ...
    <div style="padding:32px;">
      ...
      <p style="color:#6b7280;margin:0 0 24px;font-size:15px;line-height:1.6;">
        Recibimos tu pago correctamente. Tu pedido <strong style="color:#111827">#${paymentId}</strong> quedó registrado y va a estar listo para retirar.
      </p>
  ...
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'Andes Talleres <onboarding@resend.dev>', // 👈 cambiar por tu dominio cuando tengas
...
serve(async (req) => {
  try {
    ...
    // ✅ NUEVO — mandar email (no bloqueamos si falla)
if (pedidoPayload.email_cliente) {
  try {
    ...
    // No retornamos error — el pedido ya se guardó bien
  }
}
    // Borrar de temp
